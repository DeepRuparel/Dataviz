// Description: Plotting a scatterplot in d3.js which is the third plot in the index.html file. 
//with the SAT score on the x-axis and the ACT score on the y-axis. 
//The size of the circle is proportional to the GPA. The color of the circle is determined by the gpa.
...
// creating the svg using d3
let svgHeight = 500;
let svgWidth = 500;
let padding = 50;
let svg1 = d3.select("#div3")
    .append("svg")
    .attr("width", 500)
    .attr("height", 500)
    .style("border", "1px solid black")

// new data with sum of SATM and SATV for x axis and ACT for y axis, having a new field called SAT that is the sum of SATM and SATV.
let data3 = scores.map(d => {
    return {
        SATM: d.SATM,
        SATV: d.SATV,
        ACT: (450-(d.ACT)*11),
        GPA: d.GPA,
        SAT: ((d.SATM + d.SATV)/3.6)+20,
    }
}
)


// function to round and clamp values between 0 and 255
...
let circles = svg1.selectAll("circle")
    ...
    .attr("r", d=> d.GPA)
    .attr("fill", d => {
        // function to have different colors for different GPA values.
        let r, g, b
        let s, s2, s3
        let gpa = d.GPA
        if (gpa < 2){
            s = clampi(255 - (gpa*gpa)-100);
            s2 = clampi(255 - (gpa*gpa*gpa)-200);
            s3 = clampi(255- (gpa*gpa*7)-200);
        }
        else if (gpa < 2.5){
             s = clampi(255 - (gpa*gpa*2)- 200);  
             s2 = clampi(255 - (gpa*gpa*gpa)- 150);
             s3 = clampi(255- (gpa*gpa*9) - 70 );
        }
        else if (gpa < 2.7){
             s = 0;
             s2 = clampi(255 - (gpa*gpa));
             s3 = clampi(255- (gpa*gpa*5) -95);
        }
        else if (gpa < 3.0){
            s = clampi(255 - (gpa*gpa*4));
            s2 = 0;
            s3 = clampi(255- (gpa*gpa*14) - 123);
        }
        else if (gpa < 3.3){
            s = clampi(255 - (gpa*gpa*3));
            s2 =0;
            s3 = 0;
        }
        else if (gpa < 3.5){
            s = clampi(255 - (gpa*gpa*2));
            s2 = clampi(255 - gpa*4.5);
            s3 = clampi(255- (gpa*gpa*1) - 97);
        }
        else{
             s = clampi(255 - (gpa*gpa*10));
             s2 = clampi(255 - (gpa*gpa*20)+39);
            s3 = clampi(255- (gpa*gpa*7) + 67);
        }
        r = s
        g = s2
        b = s3
        return `rgb(${r}, ${g}, ${b})` // returning the rgb value
    })

// creating the x-axis and y-axis scales
let xScale = d3.scaleLinear().domain([0, 1600]).range([padding, svgWidth+10-padding])
let yScale = d3.scaleLinear().domain([0, 35]).range([svgHeight - padding, padding])

// appending the x-axis and y-axis
